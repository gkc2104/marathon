...
class CuratorElectionService(
  ...
  private[this] implicit val serialExecutionContext: ExecutionContext = ExecutionContext.fromExecutor(serialExecutor)
  ...
  private[this] def unbecomeLeader(): Unit = synchronized {
    logger.info(s"Defeated (LeaderLatchListener Interface). New leader: ${leaderHostPort.getOrElse("-")}")
  ...
  private[this] def becomeLeader(): Unit = synchronized {
    logger.info("Elected (LeaderLatchListener Interface)")
